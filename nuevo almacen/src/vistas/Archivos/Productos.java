...
void limpiar(){
    ...
    private void initComponents() {
        ...
        btnCerrar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/salir 32.png"))); // NOI18N
    ...
    private void guardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarActionPerformed
        ...
        if (control == 0) {
            try {
                ...
                if (res > 0) {
                    JOptionPane.showMessageDialog(null, "se modifico con exito el producto");
                }
                cargar_tabla();
                Inhabilitar();
                limpiar();
            ...
            try {
                ps = con.prepareStatement("INSERT INTO productos(codigo, tipo_producto, detalle, unidad, stock, stock_minimo, precio_vta) VALUES(?,?,?,?,?,?,?)");
                ps.setString(1, codigo.getText());
                ps.setString(2, String.valueOf(tipo_producto.getSelectedItem()));
                ps.setString(3, detalle.getText());
                ps.setString(4, unidad.getText());
    ...
    private void tabla_productosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabla_productosMouseClicked
       ...
       Connection con = null;
        ...
        try {
            ps=con.prepareStatement("SELECT codigo, tipo_producto, detalle, unidad, stock_minimo, precio_vta FROM productos WHERE id_productos=?");
            ps.setInt(1, id);
            
            rs=ps.executeQuery();
            if(rs.next()){
               codigo.setText(rs.getString("codigo"));
               tipo_producto.setSelectedItem(rs.getString("tipo_producto"));
